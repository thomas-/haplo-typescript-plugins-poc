...
P.respond("GET,POST", "/do/haplo-simple-notification", [
    {pathElement:0, as:"workUnit", workType:"haplo_simple_notification:message", optinal: true}
], function(E, workUnit) {
 ...
 * @property {string} title - a string used for visually identifying your notification kind on pages and emails.
 * @property {string} text - a string of the message of your notification.
 * @property {string=} buttonLabel - a string of the name of the 'task complete' confirmation button.
 * @property {string=} taskNote - a string of the prompt that appears on the task list.
 * @property {string=} link - a string of a request handler path.
 * @property {string=} template - a string of an email template
 */

/**
 * Call the corresponding service to fetch details using workUnit tags
 *
 * @param {Haplo.WorkUnit} workUnit
 * @return {NotificationDetails} - Details that declare notification spec
 */
...
P.implementService("haplo:simple_notification:query", function(kind, ref) {
    var query = O.work.query("haplo_simple_notification:message").
        tag("kind", kind);
    if(ref) {
